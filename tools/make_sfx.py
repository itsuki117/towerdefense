"""効果音の WAV を合成して assets/audio/ に書き出す。

使い方 (標準ライブラリだけで動く):

    python tools/make_sfx.py

音源を外から持ってくると再配布ライセンスの管理が要るので、
必要な数だけその場で合成している。生成物はコミットするが、
作り直したくなったらこのスクリプトを回せば同じものが出る。

音作りの方針:

- 22050 Hz / 16bit / モノラル。短いブリップばかりなので音質より容量を優先
- 矩形波は輪郭が立つので「撃つ・作る」、ノイズは「当たる・壊れる」に使う
...
def melody(notes, note_duration, shape="square", decay=9.0, volume=1.0):
...
def build():
    ...
    write("shoot_arrow", finish(mix(
        tone(0.09, 620.0, 300.0, decay=26.0),
        noise(0.02, decay=90.0, volume=0.5),
    ), peak=0.55))

    write("shoot_frost", finish(mix(
        tone(0.15, 900.0, 1500.0, shape="sine", decay=14.0),
        noise(0.15, decay=11.0, volume=0.55, smooth=0.55),
    ), peak=0.5))

    write("hit", finish(mix(
        noise(0.06, decay=48.0),
        tone(0.05, 240.0, 160.0, decay=40.0, volume=0.6),
    ), peak=0.45))

    # 撃破。ノイズで壊れる感じ、低い矩形波で重さを出す。
    write("enemy_die", finish(mix(
        noise(0.26, decay=13.0),
        tone(0.24, 280.0, 80.0, decay=11.0, volume=0.7),
    ), peak=0.7))

    # 建てた合図。上がる 2 音で「成立した」感じにする。
    write("build", finish(join(
        tone(0.07, 392.0, decay=16.0),
        tone(0.14, 587.0, decay=11.0),
    ), peak=0.6))
    ...
    write("wave_start", finish(melody([523.25, 659.25, 783.99], 0.12, decay=10.0), peak=0.65))
    write("victory", finish(melody([523.25, 659.25, 783.99, 1046.50], 0.16, decay=6.5), peak=0.8))
    write("defeat", finish(melody([392.0, 329.63, 261.63, 196.0], 0.17, shape="saw", decay=5.0), peak=0.8))
