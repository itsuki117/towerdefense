...
## 相手の最大 HP のこの割合を、上のダメージに上乗せする。
##
## 固定ダメージだけだと、波が進んで敵が硬くなるほど戦士が置いていかれる。
## 実測でも、同じゴールドならタワーのほうが強い状態が最後まで直らなかった。
## **割合ぶんを混ぜると、硬い敵ほど戦士の一撃も重くなる**ので、
## 「硬い 1 体を捕まえて削る」という仕事が最後まで成立する。
@export_range(0.0, 0.2, 0.005) var damage_percent: float = 0.0
## 攻撃レート (回/秒)。
...
## 道に居座ることで、その辺の「守られている長さ」に上乗せする量 (m)。
##
## **これが戦士にしかできない仕事。** タワーは建てた場所から動かないが、
## 戦士は道の上に立って通せんぼできる。敵は経路のコストで枝を選ぶので、
## 分かれ道の片方に戦士を置くと**敵の流れをもう一方へ寄せられる**
## （RouteGraph.add_threat / remove_threat）。
## 0 にすると流れには影響しない。
@export_range(0.0, 20.0, 0.5) var threat_length: float = 6.0
## 拠点からこの距離までしか前に出ない (m)。
##
